"use client"

import { useTranslations } from "next-intl"
import { InvoiceStatusBadge } from "./InvoiceStatusBadge"
import { ChevronRight, FileText, Calendar, Inbox } from "lucide-react"
import { format } from "date-fns"
import { cn } from "@/lib/utils"

interface InvoicesTableProps {
  invoices: any[]
  loading: boolean
  onViewDetails: (invoice: any) => void
}

export function InvoicesTable({ invoices, loading, onViewDetails }: InvoicesTableProps) {
  const t = useTranslations()

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary-200 border-t-primary-600"></div>
        <p className="mt-4 text-sm font-medium text-secondary-500">{t("portal.financials.loading") || "Loading invoices..."}</p>
      </div>
    )
  }

  if (invoices.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center px-4">
        <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-secondary-100/50 text-secondary-400 dark:bg-secondary-800/50 dark:text-secondary-500">
          <Inbox className="h-8 w-8" />
        </div>
        <h3 className="text-lg font-bold text-secondary-900 dark:text-white">
          {t("portal.financials.empty.title") || "No Invoices Found"}
        </h3>
        <p className="mt-1 text-sm text-secondary-500 max-w-sm">
          {t("portal.financials.empty.description") || "You do not have any invoices matching your criteria."}
        </p>
      </div>
    )
  }

  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm rtl:text-right">
        <thead className="border-b border-secondary-200/80 bg-secondary-50/75 text-[11px] font-bold uppercase tracking-wider text-secondary-600 dark:border-secondary-800 dark:bg-secondary-900/50 dark:text-secondary-400">
          <tr>
            <th className="px-5 py-4">{t("portal.financials.table.invoiceNumber") || "Invoice #"}</th>
            <th className="px-4 py-4">{t("portal.financials.table.date") || "Issue Date"}</th>
            <th className="px-4 py-4">{t("portal.financials.table.dueDate") || "Due Date"}</th>
            <th className="px-4 py-4">{t("portal.financials.table.total") || "Total"}</th>
            <th className="px-4 py-4 w-48">{t("portal.financials.table.payment") || "Payment"}</th>
            <th className="px-4 py-4">{t("portal.financials.table.status") || "Status"}</th>
            <th className="px-5 py-4 text-right rtl:text-left">{t("portal.financials.table.actions") || "Actions"}</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-secondary-100 dark:divide-secondary-800/60">
          {invoices.map((inv) => {
            const total = inv.totalAmount || 0
            const paid = inv.paidAmount || 0
            const percentage = total > 0 ? Math.round((paid / total) * 100) : 0
            
            return (
              <tr 
                key={inv._id} 
                className="group cursor-pointer transition-colors hover:bg-secondary-50/60 dark:hover:bg-secondary-800/40"
                onClick={() => onViewDetails(inv)}
              >
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-secondary-400" />
                    <span className="font-mono font-bold text-primary-600 dark:text-primary-400">
                      {inv.invoiceNumber}
                    </span>
                  </div>
                </td>
                <td className="px-4 py-4 text-secondary-600 dark:text-secondary-400">
                  <div className="flex items-center gap-1.5 text-xs">
                    <Calendar className="h-3.5 w-3.5" />
                    {inv.issueDate ? format(new Date(inv.issueDate), "MMM d, yyyy") : "-"}
                  </div>
                </td>
                <td className="px-4 py-4 text-secondary-600 dark:text-secondary-400">
                  <div className="flex items-center gap-1.5 text-xs">
                    <Calendar className="h-3.5 w-3.5" />
                    {inv.dueDate ? format(new Date(inv.dueDate), "MMM d, yyyy") : "-"}
                  </div>
                </td>
                <td className="px-4 py-4">
                  <span className="font-bold text-secondary-900 dark:text-white">
                    ${total.toLocaleString()}
                  </span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between text-[10px] font-bold">
                      <span className="text-secondary-500">{percentage}% {t("portal.financials.metrics.paid") || "Paid"}</span>
                      <span className="text-secondary-700 dark:text-secondary-300">
                        ${(inv.remainingAmount || 0).toLocaleString()} {t("portal.financials.metrics.remaining") || "Remaining"}
                      </span>
                    </div>
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary-100 dark:bg-secondary-800">
                      <div 
                        className={cn("h-full rounded-full transition-all", percentage === 100 ? "bg-emerald-500" : percentage > 0 ? "bg-blue-500" : "bg-transparent")}
                        style={{ width: `${percentage}%` }}
                      />
                    </div>
                  </div>
                </td>
                <td className="px-4 py-4">
                  <InvoiceStatusBadge status={inv.status} />
                </td>
                <td className="px-5 py-4 text-right rtl:text-left">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      onViewDetails(inv)
                    }}
                    className="inline-flex items-center justify-center rounded-xl border border-secondary-200 bg-white px-3 py-1.5 text-xs font-semibold text-secondary-700 shadow-sm transition-all hover:bg-secondary-100 hover:text-secondary-900 dark:border-secondary-700 dark:bg-secondary-800 dark:text-secondary-300 dark:hover:bg-secondary-700 dark:hover:text-white"
                  >
                    <span>{t("portal.financials.table.view") || "View"}</span>
                    <ChevronRight className="ml-1 h-3.5 w-3.5 rtl:mr-1 rtl:ml-0 rtl:rotate-180" />
                  </button>
                </td>
              </tr>
            )
          })}
        </tbody>
      </table>
    </div>
  )
}
