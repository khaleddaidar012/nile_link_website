  ...
  return (
    <div className="flex flex-col gap-4 border-b border-secondary-200/80 bg-secondary-50/50 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-secondary-800 dark:bg-secondary-900/50">
      ...
      <div className="relative w-full sm:max-w-xs">
        ...
        <input
          type="text"
          placeholder={t("portal.quotes.searchPlaceholder") || "Search quote # or request..."}
          value={searchQuery}
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex items-center">
          ...
          >
            <option value="all">{t("portal.quotes.filters.all") || "All Statuses"}</option>
            <option value="pending">{t("portal.quotes.status.pending") || "Pending"}</option>
            <option value="sent">{t("portal.quotes.status.sent") || "Sent"}</option>
            <option value="accepted">{t("portal.quotes.status.accepted") || "Accepted"}</option>
            <option value="rejected">{t("portal.quotes.status.rejected") || "Rejected"}</option>
          </select>
        </div>
