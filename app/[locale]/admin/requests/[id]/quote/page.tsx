...
export default function AdminGenerateQuotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        toast.success(isAr ? "تم إنشاء عرض السعر وإرساله للعميل!" : "Quote generated and sent to customer!")
        router.push(`/admin/requests/${id}`)
      } else {
        toast.error(data.error || (isAr ? "فشل إنشاء عرض السعر" : "Failed to generate quote"))
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col pb-12">
      <AdminHeader
        title={isAr
          ? `إنشاء عرض سعر للطلب: ${request?.trackingNumber}`
          : `Generate Quote for ${request?.trackingNumber}`
        }
        subtitle={isAr
          ? "راجع الأسعار المقترحة وأكمل إعداد العرض."
          : "Review suggested prices and finalize the quote."
        }
      />
      ...
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 max-w-5xl mx-auto w-full space-y-6">
        <Link href={`/admin/requests/${id}`}>
          <Button type="button" variant="outline" className="rounded-xl text-xs mb-4">
            <ArrowLeft className={`${isAr ? "ml-1.5 rotate-180" : "mr-1.5"} h-3.5 w-3.5`} />
            {isAr ? "العودة إلى الطلب" : "Back to Request"}
          </Button>
        </Link>
        ...
        <div className="bg-white dark:bg-secondary-900 rounded-xl border border-gray-200 dark:border-secondary-800 p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium mb-1">
                {isAr ? "ملاحظات للعميل" : "Notes for Customer"}
              </label>
              <textarea
                value={notes}
                ...
                rows={3}
                className="w-full p-2.5 border rounded-lg dark:bg-secondary-800 dark:border-secondary-700"
                placeholder={isAr ? "أي شروط أو أحكام خاصة..." : "Any special terms or conditions..."}
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-secondary-900 rounded-xl border border-gray-200 dark:border-secondary-800 p-6 shadow-sm">
          <h2 className="text-lg font-bold mb-4">
            {isAr ? "بنود الخدمات" : "Service Line Items"}
          </h2>
          ...
          <div className="space-y-4">
            {request?.services.map((service: any) => {
              ...
              const serviceLabel = (service.serviceKey || service.serviceType || "service").replace(/_/g, " ")

              return (
                <div key={service._id} className="p-4 border rounded-lg bg-gray-50 dark:bg-secondary-800/50 dark:border-secondary-700">
                  <h3 className="font-bold capitalize text-primary-700 mb-3">{serviceLabel}</h3>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      <label className="block text-xs font-medium mb-1 text-secondary-500">
        ...
        <div className="flex justify-end">
          <Button
            ...
            className="bg-primary-600 text-white px-8 py-4 rounded-xl text-lg hover:bg-primary-700"
          >
            {isSubmitting
              ? (isAr ? "جاري الإنشاء..." : "Generating...")
              : (isAr ? "إنشاء وإرسال عرض السعر" : "Generate & Send Quote")
            }
          </Button>
        </div>
      </form>
