...
import { Plane, ArrowLeft, Send, CheckCircle2, Package, MapPin } from "lucide-react"
    ...
    try {
      const res = await fetch("/api/portal/requests", {
        ...
        body: JSON.stringify({
          ...
          operationType,
          subject: `Air Freight from ${originAirport} to ${destinationAirport}`,
          description: `Commodity: ${commodity}, Weight: ${weight}kg, Volume: ${volume}cbm`,
          priority,
          details: {
            originAirport,
            destinationAirport,
  ...
  return (
    <div className="flex flex-col">
      <PortalHeader
        ...
        subtitle={t("portal.requests.new.air_freight_desc") || "Fast and reliable air cargo transportation."}
      />
      ...
      <div className="p-6 sm:p-8 max-w-3xl mx-auto w-full">
        <form onSubmit={handleSubmit} className="rounded-2xl border border-secondary-200/80 bg-white p-6 sm:p-8 shadow-sm dark:border-secondary-800 dark:bg-secondary-900">
          ...
          <div className="flex items-center gap-3 border-b border-secondary-100 pb-4 mb-6 dark:border-secondary-800">
            ...
            <div>
              <h2 className="text-lg font-bold text-secondary-900 dark:text-white">{t("portal.requests.new.air_freight") || "Air Freight"}</h2>
          ...
          <div className="space-y-6">
            {step === 1 && (
              <div className="grid gap-4 sm:grid-cols-2 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div>
                  <label className="mb-1 block text-xs font-bold text-secondary-700 dark:text-secondary-300 uppercase tracking-wider">{t("portal.requests.new.air_freight_form.origin_airport") || "Origin Airport"}</label>
                  <input type="text" required placeholder={t("portal.requests.new.air_freight_form.origin_placeholder") || "e.g. Cairo International"} value={originAirport} onChange={(e) => setOriginAirport(e.target.value)} className="w-full rounded-xl border border-secondary-200 bg-secondary-50/50 p-3 text-sm font-medium dark:border-secondary-700 dark:bg-secondary-800 dark:text-white focus:border-primary-500 focus:outline-none" />
                </div>
                
                <div>
                  <label className="mb-1 block text-xs font-bold text-secondary-700 dark:text-secondary-300 uppercase tracking-wider">{t("portal.requests.new.air_freight_form.destination_airport") || "Destination Airport"}</label>
                  <input type="text" required placeholder={t("portal.requests.new.air_freight_form.destination_placeholder") || "e.g. Dubai International"} value={destinationAirport} onChange={(e) => setDestinationAirport(e.target.value)} className="w-full rounded-xl border border-secondary-200 bg-secondary-50/50 p-3 text-sm font-medium dark:border-secondary-700 dark:bg-secondary-800 dark:text-white focus:border-primary-500 focus:outline-none" />
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="grid gap-4 sm:grid-cols-2 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="sm:col-span-2">
                  ...
                  <input type="text" required placeholder={t("portal.requests.new.sea_freight_form.commodity_placeholder") || "e.g. Electronics"} value={commodity} onChange={(e) => setCommodity(e.target.value)} className="w-full rounded-xl border border-secondary-200 bg-secondary-50/50 p-3 text-sm font-medium dark:border-secondary-700 dark:bg-secondary-800 dark:text-white focus:border-primary-500 focus:outline-none" />
                </div>
                ...
                <div>
                  ...
                  <input type="number" required placeholder="e.g. 500" value={weight} onChange={(e) => setWeight(e.target.value)} className="w-full rounded-xl border border-secondary-200 bg-secondary-50/50 p-3 text-sm font-medium dark:border-secondary-700 dark:bg-secondary-800 dark:text-white focus:border-primary-500 focus:outline-none" />
                </div>

                <div>
                  <label className="mb-1 block text-xs font-bold text-secondary-700 dark:text-secondary-300 uppercase tracking-wider">{t("portal.requests.new.air_freight_form.volume") || "Total Volume (CBM)"}</label>
                  <input type="number" step="0.01" required placeholder={t("portal.requests.new.air_freight_form.volume_placeholder") || "e.g. 2.5"} value={volume} onChange={(e) => setVolume(e.target.value)} className="w-full rounded-xl border border-secondary-200 bg-secondary-50/50 p-3 text-sm font-medium dark:border-secondary-700 dark:bg-secondary-800 dark:text-white focus:border-primary-500 focus:outline-none" />
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="rounded-xl bg-secondary-50 p-4 dark:bg-secondary-800/50">
                  ...
                  <div className="grid grid-cols-2 gap-y-3 text-sm">
                    ...
                    <div className="font-bold text-secondary-900 dark:text-white">{originAirport} ➔ {destinationAirport}</div>
                    
                    <div className="text-secondary-500">{t("portal.requests.new.sea_freight_form.commodity") || "Commodity"}</div>
                    <div className="font-bold text-secondary-900 dark:text-white">{commodity} ({weight} kg, {volume} CBM)</div>
                  </div>
                </div>
